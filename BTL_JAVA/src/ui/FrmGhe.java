...
public class FrmGhe extends JFrame implements ActionListener, MouseListener{
	...
	public FrmGhe() throws ParseException  {
	...
	public JPanel taoFrmPhong(){
		...
		boxLayout.add(new JLabel("Mã ghế"));
		boxLayout.add(txtMa = new JTextField(30));
		boxLayout.add(Box.createVerticalStrut(20));
		
		boxLayout.add(new JLabel("Trạng thái"));
		JRadioButton radio1 = new JRadioButton("Ghế trống");
	    JRadioButton radio2 = new JRadioButton("Ghế đã đặt");
	    radio1.setActionCommand("Ghế trống");
        radio2.setActionCommand("Ghế đã đặt");
        radio1.setSelected(true);
	    ButtonGroup group = new ButtonGroup();
	    group.add(radio1);
	    group.add(radio2);
	    boxLayout.add(radio1);
	    boxLayout.add(radio2);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(o == btnThem) {
			...
			String soLuongGhe = txtTrangThai.getText().trim();
...
//			btnThem.setEnabled(true);
			...
			if(ma.isEmpty() || ten.isEmpty() || soLuongGhe.isEmpty()) {
				...
				if (validData()) {
					...
					if(phongDAO.updatePhong(data)) {
						...
						table.setValueAt(txtTrangThai.getText(), row, 2); 
	...
	private boolean validData() {
		String ma = txtMa.getText().trim();
		String ten = txtTen.getText().trim();
		String soLuong = txtTrangThai.getText().trim();
